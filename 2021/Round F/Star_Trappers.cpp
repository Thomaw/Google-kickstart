...
void solve(int tc) {
    ...
    // dbg_time();
	re(N); star.rsz(N); re(star);
	P blue; re(blue); each(t,star) t -= blue;
	sort(all(star),angleCmp);
	// dbg(star);
	V<vd> dist(N,vd(N,BIG));
	F0R(i,N) F0R(j,N) if (cross(star[i],star[j]) > 0) {
		ckmin(dist[i][j],abs(star[i]-star[j]));
		// dbg("EDGE",i,j);
	}
	F0R(k,N) F0R(i,N) F0R(j,N) ckmin(dist[i][j],dist[i][k]+dist[k][j]);
	db ans = BIG; F0R(i,N) ckmin(ans,dist[i][i]);
	if (ans == BIG) ps("IMPOSSIBLE");
	else ps(ans);
}
